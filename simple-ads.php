<?php
/*
Plugin Name: PS Simple Ads
Plugin URI: https://n3rds.work/shop/artikel/ps-simple-ads-plugin/
Description: Dieses Plugin übernimmt die Werbebasics, es fügt Deinen Posts und Beiträgen Werbeanzeigen hinzu, zB. Google Adsense oder Du erstellst eigene.
Author: WMS N@W
Version: 1.0.6
...
function simple_ads_network_pages() {
	...
	if ( version_compare($wp_version, '3.0.9', '>') ) {
	    if ( is_network_admin() ) {
		add_submenu_page($simple_ads_settings_page, __('Werbeanzeige', 'simple_ads'), __('Werbeanzeige', 'simple_ads'), 'manage_network_options', 'advertising', 'simple_ads_site_output');
	    }
	} else {
	    if ( is_super_admin() ) {
		add_submenu_page($simple_ads_settings_page, __('Werbeanzeige', 'simple_ads'), __('Werbeanzeige', 'simple_ads'), 10, 'advertising', 'simple_ads_site_output');
	    }
	}
}
...
function simple_ads_pages() {
	...
	add_submenu_page($simple_ads_settings_page, __('Werbeanzeige', 'simple_ads'), __('Werbeanzeige', 'simple_ads'), 'manage_options', 'advertising', 'simple_ads_site_output');
}
...
function simple_ads_site_output() {
	...
	if(!current_user_can('manage_options')) {
		echo "<p>" . __('Netter Versuch...', 'simple_ads') . "</p>";  //If accessed properly, this message doesn't appear.
		return;
	}
	...
	switch( $_GET[ 'action' ] ) {
		...
		default:
            ...
            <?php _e('Vor dem Beitragsinhalt', 'simple_ads'); ?></label>
            <br />
            ...
            <?php _e('Nach dem Beitragsinhalt', 'simple_ads'); ?></label>
            <br />
            ...
            <?php _e('Vor dem Seiteninhalt', 'simple_ads'); ?></label>
            <br />
            ...
            <?php _e('Nach dem Seiteninhalt', 'simple_ads'); ?></label>
            </td>
            </tr>
            <tr valign="top">
            <th scope="row"><?php _e('Anzeigen pro Seite', 'simple_ads') ?></th>
            <td>
            <?php
            ...
            <br /><?php _e('Maximale Anzahl von Anzeigen, die auf einer einzelnen Seite geschaltet werden sollen. Setze dies für Google Adsense auf "3".', 'simple_ads') ?></td>
            </tr>
            <tr valign="top">
            <th scope="row"><?php _e('"Bevor" Anzeigeninhalt (HTML)', 'simple_ads') ?></th>
            <td>
            ...
            <br /><?php _e('Wird vor dem Beitrags- und Seiteninhalt verwendet.', 'simple_ads') ?></td>
            </tr>
            <tr valign="top">
            <th scope="row"><?php _e('"Dannach" Anzeigeninhalt (HTML)', 'simple_ads') ?></th>
            <td>
            ...
            <br /><?php _e('Wird nach dem Inhalt von Beiträgen und Seiten verwendet.', 'simple_ads') ?></td>
            </tr>
            <tr valign="top">
            <th scope="row"><?php _e('Hauptseite (Multisite)', 'simple_ads') ?></th>
            <td>
            <?php
            $advertising_main_blog = get_site_option('advertising_main_blog', 'hide');
			?>
            <select name="advertising_main_blog" id="advertising_main_blog" >
                <option value="hide" <?php if ( $advertising_main_blog == 'hide' ) { echo 'selected="selected"'; } ?> ><?php _e('Anzeigen ausblenden', 'simple_ads'); ?></option>
                <option value="show" <?php if ( $advertising_main_blog == 'show' ) { echo 'selected="selected"'; } ?> ><?php _e('Anzeigen anzeigen', 'simple_ads'); ?></option>
            </select>
            <br /><?php //_e('') ?></td>
            </tr>
            </table>
            ...
            <p class="submit">
            <input class="button button-primary" type="submit" name="Submit" value="<?php _e('Änderungen speichern', 'simple_ads') ?>" />
			<input class="button button-secondary" type="submit" name="Reset" value="<?php _e('Zurücksetzen', 'simple_ads') ?>" />
            </p>
            </form>
			<?php
		break;
		...
		case "process":
			if ( isset( $_POST[ 'Reset' ] ) ) {
				...
				window.location='{$simple_ads_settings_page}?page=advertising&updated=true&updatedmsg=" . urlencode(__('Änderungen gespeichert.', 'simple_ads')) . "';
				</script>
				";
			} else {
...
$MyUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
	'https://n3rds.work//wp-update-server/?action=get_metadata&slug=simple-ads', 
	__FILE__, 
	'simple-ads' 
);
